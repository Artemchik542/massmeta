...
Преполагается, что у Вас есть первичный опыт работы с C/Java и/или JavaScript подобным кодом, а также с Git-подобными системами контроля версий. Будет круто, если у Вас уже есть опыт программирования на здешнем языке DM и/или опыт работы с React JS, на чем собственно и основан интерфейл TGUI.
...
Если модуль маленький и фалов мало, то никто не мешает расположить оснеовные файлы без системы подпапок.
...
С помощью этих файлов мы **косвенно изменяем** основной код /TG/station. Этот подход позволяем нам быстрее обновлять репозиторий.

За счёт отсуствия изменений в основном коде, не нарушается череда оригинальных коммитов, и тем самым мы **не допускаем в будущем Конфликтов Слияния**.

Однако такой подход этим и грешит. **Гитхаб не сможет нам оперативно подсказать где оригинальный файл внезапно поменялся или и того хуже – удалился.**

Иногда прямое изменение кода через `//MASSMETA EDIT` предпочтительнее.

Все файлы данного типа обязательно кладете в подпапку `code/~overrides/`, это нужно для автоматической системы тестирования.
...
И теперь, если вы сделаетете подобый манёвр `. = ..()`, то Вы вызовите родительскую функцию и присвоите её значение возвращаемому значению нашей функции. После этого можете свободно дополнять свою `.` чем хотите. В примере выше как раз просходит это, добавлется дополнительное действие.
...
Также учтите, что вы не сможете при модульном дополнении функции использовать те переменные, которые были объявленны в функции оригинале! (<- !провести факт-чеккинг!)
